  ...
  private static enum Counters {
	...
	public static class Map extends MapReduceBase implements
		...
		public void map(IntWritable key, WikipediaPage page, 
				OutputCollector<PairOfIntString, Text> output, Reporter reporter) 
						throws IOException {
			if (!page.isArticle()) {
				return;
			}
			...
			for (Link link : page.extractLinks()) {	
  			...
  			if(!normalizedAnchorText.equals("")) {
  				outputKey.set(1, normalizedAnchorText);
					outputValue.set(canonicalTarget + "+" + title);
	...
	public static class Reduce extends MapReduceBase implements
		...
		public void reduce(PairOfIntString key, Iterator<Text> values,
			...
			Set<String> candidatesList = new HashSet<String>();
		...
		public int getIteratorCount(Iterator<IntWritable> values) {
			int count = 0;
			while (values.hasNext()) {
				values.next();
				++count;
	...
	public void task1(Configuration config, String inputPath, String outputPath, String titlesPath,
		...
		conf.setJobName(String.format(
				"EntityMentionDFIndexBuilderDebug:[input: %s, output: %s, titles: %s, redirect mapping: %s]", 
				inputPath, 
				outputPath,
				titlesPath,
				redirectMapPath
